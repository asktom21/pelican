...
pg_dump -h host_name -d appdb -U postgres -f /home/postgres/backup_rds_dump/appdb.$(date +%Y%m%d).dump
# restore command: psql -f appdb.$(date +%Y%m%d).dump -h localhost -d appdb -U postgres

# backup command (custom format):
pg_dump -h host_name -d appdb -U postgres -Fc -f /home/postgres/backup_rds_dump/appdb.$(date +%Y%m%d).Fc.dump
